...
2. In Unity Hub, choose **Add → Add project from disk** and select the project folder. Open it with Unity version **6000.4.6f1**.
3. In the Project window, open the **Scenes** folder and open the **MainMenu** scene.
4. Once the scene finishes loading, press **Play**, then click **Play** on the main menu to start a run.
...
1. **Slow zones** reduce the ball's speed for a short time.
2. **Slippery zones** reduce steering control, making the ball harder to maneuver.
3. **Moving saw obstacles** end the run immediately on contact.
